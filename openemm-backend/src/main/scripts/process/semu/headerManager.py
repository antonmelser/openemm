                ...
                for cline in urimatrix.split('\n'):
                    if cline.startswith(sDomain + '|'):
                        if len(cline.split('|')) == 3:
                            ...
                            isInMatrix = True
                            break
                        elif len(cline.split('|')) == 2:
                            mail.head.append('List-Unsubscribe: <%s>' % (cline.split('|')[1] % data, ))
                            isInMatrix = True
                            break
...
if __name__ == '__main__':
    def _main():
        ...
        mail = struct()
        mail.head = []
        mail.head.append('Message-ID: <20130823020049-1.1.h.b.0.pp89lw4y80@adomain.com>')
        ...
        mail.head = []
        __aps__['uri'] = 'http://myotherdomain/unsubscribe?%(urlrecv)s'
        handleOutgoingMail(None, mail)
        print mail.head[0]
